...
$(document).ready(function () {
  ...
  $(".tab-1 li").click(function () {
    let num = $(this).index();
    let move = 109 * num; // 세로폭 70px씩 증가됨
...
$(".w_list li").click(function () {
  ...
  //슬라이드 이동
  $(".w_bg ul").animate({ left: -700 * val + "px" });
  $(".w_txt_slide ul").animate({ left: -330 * val + "px" });
  $(".w_tab_slide ul").animate({ left: -794 * val + "px" });
});
...
$(document).ready(function () {
  $(".title").click(function () {
    ...
    $(this).siblings(".desc").stop().slideUp(200);
    $(this).next(".desc").stop().slideToggle(200);

    let dataImage = $(this).attr("data-image");
    $(".acimage img").attr("src", dataImage).hide().fadeIn(200);
  });
});
